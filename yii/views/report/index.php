...
//            }
        ...
        <td class="rep-column"<?= count($item['warning']) > 0 ? ' style="font-weight: 600; " ' : '' ?>>
            <?= $item['cost'] != 0 ? number_format($item['cost'], 2) : '0' ?>
            <?php $page->setCellValue("O".($No + 1), ($item['cost'] != 0 ? number_format($item['cost'], 2) : 0)); ?>
            <?php if (count($item['warning']) > 0)  $page->getStyle("O".($No + 1))->applyFromArray($arBoldStyle); ?>
