<style>
    ...
    #load {
        ...
        margin-top:-48px;
        background: url(../web/files/images/load.svg) no-repeat;
    }
    .pos-1 {
        position: absolute;
        margin-left: -4px;
...
<div id="modal">
    ...
    <center>
        ...
        <div style="margin: 3px; width: 30%; float: right; margin-top: 32px; margin-bottom: 24px; ">
        ...
        <table id="modal-table" style="width: 90%; margin-top: 16px; ">
            <thead>
                <tr>
            ...
            <tfoot>
                <tr>
                    ...
                    <td>
                        <button class="btn btn-success" onclick="addModalTableRow(); " >+</button>
                    </td>
                </tr>
            </tfoot>
        </table>
        <div id="modal-footer">
            <button class="btn btn-success pull-left">Сохранить</button>
...
<?php //$this->title = "Сигнал: Расчёт зарплаты" ?>
            ...
            switch (sBase) {
                case 'salary': 
                    switch (sDep){
                        ...
                        break;
                        case 'fixed': row_total = row_total + salary;
                        break;
                ...
                case 'award':
                    switch (sDep){
                        ...
                        break;
                        case 'fixed': row_total = row_total + award;
                        break;
                ...
                case 'summa':
                    switch (sDep){
                        ...
                        break;
                        case 'fixed': row_total = row_total + summa;
                        break;
...
//                        copySalaryFromPrevMonth(this);
        ...
        if (salary && salary['payment'] && !salary['block']) {
            ...
            switch(base) {
                case 'salary': // основание "оклад"
                    switch(depends) {
                        ...
                        case 'fixed': // коэффициент "фиксированная сумма" (не зависит от коэффициента)
                            result['base'] = result['base'] + bs_pay;
                ...
                case 'award': // основание "премия"
                    switch(depends) {
                        ...
                        break;
                        case 'overtime': // коэффициент "переработка"
                            result['award'] = result['award'] + Math.ceil(award * _shour / phour);
                        ...
                        case 'taskcount': // коэффициент "количество задач" - количество задач умноженное на среднечасовой оклад
                            result['award'] = result['award'] + Math.ceil(award * cost / phour); //award / phour;
                            result['title'] = result['title'] + info + ': ' + award + '*' + cost + '/' + phour + ' = ' + Math.ceil(award * cost / phour) + '\n';
                        break;
                        case 'fixed': // коэффициент "фиксированная сумма" (не зависит от коэффициента)
                            result['award'] = result['award'] + award * 1;
...
<template id="tmp-row" >
    <tr class="modal-table-row">
        <td>
            <input type="text" class="form-control" placeholder="Название выплаты" autofocus="autofocus" />
        </td>
        <td>
        ...
        <td></td>
        <td>
            <a class="btn btn-danger" onclick="removeModalTableRow(this); ">Х</a>
        </td>
    </tr>
</template>
